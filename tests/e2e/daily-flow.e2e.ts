...
test('logs a complete day, finalizes it, and restores it after reload', async ({ page }) => {
  ...
  await page.getByRole('spinbutton', { name: 'Running time min', exact: true }).fill('30')
  ...
  await page.reload()
  await expect(page.getByRole('heading', { name: 'Day complete', exact: true })).toBeVisible()
  ...
  await expect(page.getByRole('spinbutton', { name: 'Running time min', exact: true })).toHaveValue('30')
